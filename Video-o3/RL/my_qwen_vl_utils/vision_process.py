...
def fetch_video_raw_with_scenes(ele: dict) -> tuple[torch.Tensor, float, list[tuple[int, int]]]:
    ...
    Returns:
        tuple: (video_tensor, sample_fps, scene_boundaries)
            - video_tensor: 原始视频张量 (T, C, H, W)
            - sample_fps: 采样FPS
            - scene_boundaries: 每个场景的 (start_frame, end_frame) 元组列表
    """
    ...
    try:
        ...
        logger.warning(f"视频读取后端 {video_reader_backend} 出错，使用torchvision作为默认，错误信息: {e}")
        video, sample_fps = VIDEO_READER_BACKENDS["torchvision"](ele)

    # 检测场景边界
    boundaries = detect_scene_boundaries(video, percentile=90)
    scenes = group_frames_by_scene(video, boundaries)

    return video, sample_fps, scenes
...
def process_video_adaptive_token_num_with_scenes(
    ...
    Args:
        ...
        min_tokens: 每帧最小令牌数
        fps: 处理后视频的目标FPS
        source_frames_fps: 源帧的原始FPS
        detect_scenes: 是否检测场景边界

    Returns:
        tuple: (video, sample_fps, raw_video, raw_sample_fps, scene_boundaries)
            - video: 处理后的视频张量 (T, C, H, W)
            - sample_fps: 处理后视频的采样FPS
            - raw_video: 原始视频张量 (T_raw, C, H, W)
            - raw_sample_fps: 原始视频的采样FPS
            - scene_boundaries: 处理后视频帧中的 (start_frame, end_frame) 元组列表
    """
    from my_qwen_vl_utils.scene_detection import (
        detect_scene_boundaries, group_frames_by_scene, map_raw_scenes_to_processed
    )

    assert os.path.exists(video_path), f"视频路径 {video_path} 不存在。"
    ...
    try:
        ...
        # 在原始视频中检测场景
        if detect_scenes:
            raw_boundaries = detect_scene_boundaries(raw_video, percentile=90)
            raw_scenes = group_frames_by_scene(raw_video, raw_boundaries)
        else:
            raw_scenes = [(0, raw_video.shape[0])]
        ...
        # 将原始场景映射到处理后的视频帧
        if detect_scenes:
            processed_scenes = map_raw_scenes_to_processed(
                raw_scenes,
                raw_sample_fps,
                sample_fps,
                video.shape[0]
            ...
            processed_scenes = [(0, video.shape[0])]

        return video, sample_fps, raw_video, raw_sample_fps, processed_scenes

    except Exception as exn:
        print(f"处理视频 {video_path} 失败。异常: {exn}")
        raise exn
