...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _regularize_videos_av(self, videos: list["VideoInput"], **kwargs) -> "RegularizedVideoOutput":
        results, fps_per_video, durations, crop_intervals, sample_strategies, scene_boundaries_list = [], [], [], [], [], []
        use_scene_detection = os.environ.get('USE_SCENE_DETECTION', 'false').lower() == 'true'  # 从环境变量启用场景检测
        ...
        for idx_video, _video in enumerate(videos):
            ...
            scene_boundaries_list.append(scene_boundaries)
    ...
    def _regularize_videos_decord(self, videos: list["VideoInput"], **kwargs) -> "RegularizedVideoOutput":
        r"""规范化视频以避免使用decord时的错误。包括读取、调整大小和转换。"""
        results, fps_per_video, durations, crop_intervals, sample_strategies, scene_boundaries_list = [], [], [], [], [], []
        use_scene_detection = os.environ.get('USE_SCENE_DETECTION', 'false').lower() == 'true'  # 从环境变量启用场景检测
        ...
        for idx_video, _video in enumerate(videos):
            ...
            try:
                ...
                if use_scene_detection:
                    try:
                        from my_qwen_vl_utils.scene_detection import detect_scene_boundaries, group_frames_by_scene
                        all_frames_tensor = torch.stack([
                            torch.from_numpy(vr[i].asnumpy()).permute(2, 0, 1)
                            for i in range(total_frames)
                        ])
                        boundaries = detect_scene_boundaries(all_frames_tensor, percentile=90)
                        scene_boundaries = group_frames_by_scene(all_frames_tensor, boundaries)
                    except Exception as e:
                        import logging
                        logger = logging.getLogger(__name__)
                        logger.warning(f"场景检测失败: {e}")
